...
==注意==：如果上传本地仓库步骤有地方错了可以用git reset HEAD^	取消已经暂存的消息，当我们对一个工程进行测试的时候，如果在测试过程中你修改的代码和添加的文件不需要，这个时候我们需要代码滚回，使用`git checkout -- .` 将所有文件滚回最近一次上传到仓库的版本，如果需要将指点的文件进行滚回只需要在后面加上文件名`git checkout -- aa.txt`就行
